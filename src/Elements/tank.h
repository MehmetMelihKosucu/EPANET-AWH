...
class Tank: public Node
{
  public:
    ...
    Tank(std::string name_);
    ~Tank() {}

    // Overridden virtual methods
    int type() const override { return Node::TANK; }
    ...
    void   setFixedGrade();
    void   updateVolume(double tstep);
    void   updateArea();
    ...
    double lastRhsTerm;            //!< last right-hand side term
    double lastDiagCoeff;           //!< last diagonal term
    double expectedHeadChange;     //!< expected head change
