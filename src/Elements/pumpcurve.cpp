...
/**
 * Helper method to get the shutoff head from a pump curve
 * Needed for pump status checks
 */
double PumpCurve::getShutoffHead(double speed) const
{
    // Shutoff head is the head at zero flow, scaled by speed^2
    if (curve) {
        // The first y-value in a pump curve is the shutoff head
        return curve->getFirstY() * speed * speed;
    }
    else if (horsepower > 0.0) {
        // For constant horsepower pumps, use hMax
        return hMax * speed * speed;
    }
    
    // Default shutoff head if no data available
    return 0.0;
}

/**
 * Get the maximum flow for the pump curve at the specified speed
 * @param speed Current pump speed (relative to design speed)
 * @return The maximum flow (at zero head), scaled by speed
 */
double PumpCurve::getMaxFlow(double speed) const
{
    // Maximum flow is at zero head, scaled by speed
    if (curve) {
        // The last x-value in a pump curve is the maximum flow
        return curve->getLastX() * speed;
    }
    else if (horsepower > 0.0) {
        // For constant horsepower pumps, use qMax
        return qMax * speed;
    }
    
    // Default max flow if no data available
    return qInit * 2.0 * speed; // Some default value based on design point
}

// Helper function to safely access pump curve data
double PumpCurve::getHeadAt(double flow) const
{
    if (!curve || curve->size() == 0) return 0.0;
    
    // Find appropriate curve point and interpolate
    if (flow <= curve->x(0)) return curve->y(0);  // At or below first point
    
    for (int i = 1; i < curve->size(); i++) {
        if (flow <= curve->x(i)) {
            // Interpolate between (i-1) and i
            double dx = curve->x(i) - curve->x(i-1);
            if (dx == 0.0) return curve->y(i-1);
            
            double ratio = (flow - curve->x(i-1)) / dx;
            return curve->y(i-1) + ratio * (curve->y(i) - curve->y(i-1));
        }
    }
