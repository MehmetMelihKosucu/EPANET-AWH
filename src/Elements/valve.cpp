...
void Valve::findHeadLoss(Network* nw, double q)
{
	...
	case CCV: 
		...
		else if (setting == 1)
		{
			status = LINK_OPEN;
			findOpenHeadLoss(q); 
            inertialTerm = MIN_GRADIENT; // 10.765 / (32.174 * PI * diameter * diameter); // Approximate value for valve inertial term
		}
        else
        {
		...
		break;
	case DPRV:
		if (setting != Valve::FO && setting != Valve::TM && setting != Valve::FM && setting != Valve::RNM)
		{
			status = LINK_CLOSED;
			HeadLossModel::findClosedHeadLoss(q, hLoss, hGrad);
			inertialTerm = MIN_GRADIENT;
...
void Valve::findDprvHeadLoss(Network* nw, double q)
{
	...
	double k3 = 2.33;
	double k4 = -0.21;
	double Cvmax = 1.442760731; // = 0.074 m^5/2;
	double Cvtr = 0.07550186203; // 0.00387253248
...
double Valve::getSegmentLength(double desiredSegmentLength, double tstep) const {
    return 0.0;  // Valves do not support segmentation
}

void Valve::initializeActive()
{
    // In active mode, we want to preserve the valve's setting if it�s already nonzero.
    // So we set the status to the initial status.
    status = initStatus;

    // Only assign the initial setting if the current setting is essentially zero.
    if (std::fabs(setting) < 1e-9) {
        setting = initSetting;
    }

    // Do NOT call setInitFlow() here because that routine resets the flow
    // based on a default computation (e.g. using 1 ft/s velocity), which we don't want.
    // Instead, we assume that the active network has already computed and stored the desired flow.

    // Update the fixed status flag based on the initial status.
    hasFixedStatus = (initStatus != VALVE_ACTIVE);

    // Update the loss factor based on the current setting and other valve parameters.
    setLossFactor();

    // Optionally, you could update additional active-specific parameters here.
    // For example, if the valve has pressure management settings, update them without overwriting the existing setting.
}
