...
// Get the first y-value in the curve (y at minimum x)
double Curve::getFirstY() const
{
    if (yData.empty()) return 0.0;
    return yData[0];
}

//-----------------------------------------------------------------------------
// Get the last x-value in the curve (maximum x)
double Curve::getLastX() const
{
    if (xData.empty()) return 0.0;
    return xData[xData.size() - 1];
}