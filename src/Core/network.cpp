...
//       already contain an element with the same name.
    ...
    try
    {
        if ( element == Element::NODE )
        {
            ...
            if (!node) {
                return false; // Factory method failed
            }
        }

        else if ( element == Element::LINK )
        {
            Link* link = Link::factory (type, name, memPool, this);
...
int Network::nodeCount() const {
    return nodes.size(); // Assuming `nodes` is a container of Node objects
}

int Network::linkCount() const {
    return links.size(); // Assuming `links` is a container of Link objects
}

// write this function's interior part properly
Options& Network::getOptions() {

    double totalDuration;
	totalDuration = options.timeOption(Options::TOTAL_DURATION);
	return options;
}

std::vector<Link*> Network::getLinksConnectedToNode(Node* node)
{
