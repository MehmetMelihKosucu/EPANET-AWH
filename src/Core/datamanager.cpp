...
int DataManager::getTimeParam(int param, long* value, Network* nw)
{
	*value = 0;
	switch (param)
	{
	case EN_DURATION:     *value = nw->getOptions().timeOption(Options::TOTAL_DURATION); break;
	case EN_HYDSTEP:      *value = nw->getOptions().timeOption(Options::HYD_STEP);      break;
	case EN_QUALSTEP:     *value = nw->getOptions().timeOption(Options::QUAL_STEP);     break;
	case EN_PATTERNSTEP:  *value = nw->getOptions().timeOption(Options::PATTERN_STEP);  break;
	case EN_PATTERNSTART: *value = nw->getOptions().timeOption(Options::PATTERN_START); break;
	case EN_REPORTSTEP:   *value = nw->getOptions().timeOption(Options::REPORT_STEP);   break;
	case EN_REPORTSTART:  *value = nw->getOptions().timeOption(Options::REPORT_START);  break;
	case EN_RULESTEP:     *value = nw->getOptions().timeOption(Options::RULE_STEP);     break;
	case EN_STATISTIC:    *value = nw->getOptions().timeOption(Options::REPORT_STATISTIC); break;
